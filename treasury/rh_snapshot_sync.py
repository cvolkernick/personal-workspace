#!/usr/bin/env python3
"""Refresh local Robinhood snapshot: prefer Pi schedule, fall back to local MCP.

Order:
  1) If Pi is reachable over SSH, pull ``robinhood_latest.json`` (and optionally
     fund-manager snapshot) from the Pi workspace.
  2) If Pi is down, remote file is missing/stale/invalid, or pull fails → run
     local Grok + Robinhood MCP refresh (same as ``rh_refresh.sh``).
...
Env (optional):
  ...
  TREASURY_RH_MAX_AGE_HOURS    accept remote only if as_of younger than this (default 12)
  TREASURY_SKIP_PI=1           force local MCP only
  TREASURY_SKIP_LOCAL_MCP=1    do not fall back to grok/MCP

Config (treasury/config.json → pi_sync):
  ssh, remote_root, connect_timeout_s, max_age_hours, enabled
...
def refresh_via_local_mcp(*, timeout_s: float = 90.0) -> Dict[str, Any]:
    ...
    if r.returncode != 0 and not out["ok"]:
...
def sync_rh_snapshot(
    *,
    ...
    """Pi first, then local MCP. Returns combined status."""
    result: Dict[str, Any] = {
        "ok": False,
        "source": None,
        "pi": None,

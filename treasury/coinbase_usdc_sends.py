...
Thaís standing (Chris 2026-09-13): dest AwMH3… $900 monthly on the 10th.
$415 biweekly is no longer the post-9/11 rule. Pre-standing dest-only $895
(2026-08-10) and name-on-address still join — #431/#432 August must not blank.
Weekly $208 / daily $25 after 2026-08-30 / 2026-09-04 never paint.
Proof ids baa3976e and 7b8bf83b stay excluded.

Rent standing stays $350 one row every 14d from 2026-09-11 1:00 PM ET.
JR self-send dest CzuRx… is $35, treated as a manual Friday reminder —
match $35 dest sends if present; do not invent automatic biweekly $70.
JR is not Rent / Thaís / mint. Rent planned stays sheet monthly —
never rewrite to $25 * days or 14×$25.
...
# Standing dest/cadence. Do not invent other dests.
...
THAIS_STANDING_AMOUNT = 900.0
# Dead post-9/11 biweekly — not the standing rule after 2026-09-13.
THAIS_BIWEEKLY_LEGACY_AMOUNT = 415.0
# 2026-08-10 dest-only monthly. Keep so #431/#432 August paint is not blanked.
THAIS_PRE_STANDING_AMOUNT = 895.0
THAIS_DEAD_WEEKLY_AMOUNT = 208.0
THAIS_NEVER_PAINT_DATE = date(2026, 9, 4)
THAIS_NETWORK = "solana"
THAIS_MONTHLY_DAY = 10
...
JR_SELF_SEND_AMOUNT = 35.0
JR_SELF_SEND_LABEL = "self-send"
JR_SELF_SEND_NETWORK = "solana"
JR_REMINDER_CADENCE = "manual_friday_reminder"
...
# Thaís monthly standing starts 2026-09-10 (not the Rent pay-Friday grid).
THAIS_STANDING_FIRST = datetime(2026, 9, 10, 13, 0, 0, tzinfo=PAY_FRIDAY_TZ)
THAIS_STANDING_FIRST_DATE = THAIS_STANDING_FIRST.date()
...
def matches_thais(tx: Dict[str, Any]) -> bool:
    """Thaís = dest AwMH3… $900 monthly on the 10th, else pre-standing dest/named.

    Dest ~$900 (incl. 2026-09-10) joins. $415 biweekly is not the post-9/11 rule.
    August dest-only $895 still joins (#432). Weekly $208 never paints.
    2026-09-04 never paints. Proof id excluded.
    """
    ...
    # $415 biweekly is no longer the post-9/11 standing rule.
    if dest_hit and _near_amount(amt, THAIS_BIWEEKLY_LEGACY_AMOUNT):
        return False
    # Standing monthly $900 dest. Sep 10 2026 must join.
    if dest_hit and _near_amount(amt, THAIS_STANDING_AMOUNT):
        if not network_ok(tx, THAIS_NETWORK):
            return False
        return True
    # Pre-standing dest-only August 895. Do not paint $895 as Sep monthly.
    if dest_hit and _near_amount(amt, THAIS_PRE_STANDING_AMOUNT):
        return d is None or d < THAIS_STANDING_FIRST_DATE
    if d is not None and d >= THAIS_STANDING_FIRST_DATE:
        return False
...
def matches_jr_self_send(tx: Dict[str, Any]) -> bool:
    """JR self-send = dest CzuRx… $35 if present. Not automatic $70. Not Rent / Thaís / mint."""
    if is_excluded_send_id(tx):
        return False
    if dest_matches_address(tx, THAIS_DEST):
        return False
    ...
    if not network_ok(tx, JR_SELF_SEND_NETWORK):
        return False
    amt = send_spend_amount(tx)
    # Manual Friday reminder amount. Do not invent automatic biweekly $70.
    return _near_amount(amt, JR_SELF_SEND_AMOUNT)
...
def next_thais_monthly(as_of: Any = None) -> datetime:
    """Next Thaís $900 monthly on the 10th at 1:00 PM ET."""
    now = _as_et(as_of)
    if now <= THAIS_STANDING_FIRST:
        return THAIS_STANDING_FIRST
    candidate = datetime(
        now.year, now.month, THAIS_MONTHLY_DAY, 13, 0, 0, tzinfo=PAY_FRIDAY_TZ
    )
    if now <= candidate:
        return candidate
    if now.month == 12:
        return datetime(now.year + 1, 1, THAIS_MONTHLY_DAY, 13, 0, 0, tzinfo=PAY_FRIDAY_TZ)
    return datetime(now.year, now.month + 1, THAIS_MONTHLY_DAY, 13, 0, 0, tzinfo=PAY_FRIDAY_TZ)


def next_friday_reminder(as_of: Any = None) -> datetime:
    """Next Friday 1:00 PM ET reminder. Not an automatic $70 send."""
    now = _as_et(as_of)
    days_ahead = (4 - now.weekday()) % 7
    candidate = datetime(now.year, now.month, now.day, 13, 0, 0, tzinfo=PAY_FRIDAY_TZ)
    ...
    return (
        {
            ...
            "cadence": "monthly_on_10th",
            "cadence_days": None,
            "first_at": THAIS_STANDING_FIRST.isoformat(),
        ...
        },
        {
            ...
            "cadence": JR_REMINDER_CADENCE,
            "automatic": False,
            "first_at": PAY_FRIDAY_FIRST.isoformat(),
            "tz": PAY_FRIDAY_TZ_NAME,
            "book_as": "jr_self_send",
            "not": ("rent", "thais", "mint"),
            "notes": "manual Friday reminder — not automatic biweekly $70",
        },
    )


def _next_for_standing(row: Dict[str, Any], as_of: Any = None) -> datetime:
    kind = str(row.get("kind") or "")
    if kind == "thais":
        return next_thais_monthly(as_of)
    if kind == "jr_self_send":
        return next_friday_reminder(as_of)
    return next_pay_friday(as_of)
...
def next_standing_sends(as_of: Any = None) -> List[Dict[str, Any]]:
    ...
    for row in standing_send_defs():
        rec = dict(row)
        nxt = _next_for_standing(rec, as_of)

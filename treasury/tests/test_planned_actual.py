...
class TestStandingSendJoin427(unittest.TestCase):
    def test_sep_strip_joins_standing_dests_and_keeps_sheet_planned(self) -> None:
        ...
        rent = _row(strip, "Rent")
        self.assertAlmostEqual(thais["planned"], 900.0)
        self.assertAlmostEqual(thais["actual"], 900.0)
        self.assertGreaterEqual(thais["tx_count"], 1)
        self.assertEqual(thais["flag"], FLAG_ON)
        self.assertEqual(thais["from"], COINBASE_USDC_LABEL)
        self.assertEqual(thais["next_send"]["amount"], 900.0)
    ...
    def test_jr_sheet_item_joins_when_already_planned(self) -> None:
        items = [
            _item("Rent", 2090.0, "Coinbase"),
            _item("Thaís", 900.0, "Coinbase"),
            _item("JR self-send", 35.0, "Coinbase"),
        ]
        cmap = _ac_map()
        ...
        rent = _row(strip, "Rent")
        self.assertAlmostEqual(jr["actual"], 35.0)
        ...
        self.assertEqual(rent["actual"], 0.0)
        self.assertNotEqual(rent["actual"], 35.0)
    ...
    def test_next_standing_paints_before_sep11(self) -> None:
        ...
        self.assertEqual(thais["actual"], 0.0)
        self.assertEqual(thais["next_send"]["amount"], 900.0)
        self.assertEqual(thais["next_send"]["next_date"], "2026-09-10")
        self.assertEqual(rent["next_send"]["amount"], 350.0)
        jr = [s for s in strip["standing_sends"] if s["kind"] == "jr_self_send"][0]
        self.assertEqual(jr["amount"], 35.0)
        self.assertEqual(jr["label"], "self-send")
        self.assertEqual(jr["next_date"], "2026-09-04")
        self.assertEqual(jr["cadence"], "manual_friday_reminder")


class TestAugustThaisDestMatcher432(unittest.TestCase):
    """#432: August dest-only 895 paints ON. Dest $900 monthly is the 10th."""
    ...
    def test_sep10_dest_900_joins_after_aug_restore(self) -> None:
        snaps = _snaps(_ac_items(), [])
        snaps["coinbase_usdc_sends"] = {
            "source": "coinbase_v2_usdc",
            "transactions": [
        ...
        sep = build_planned_actual_strip(snaps, _ac_map(), as_of="2026-09-10")

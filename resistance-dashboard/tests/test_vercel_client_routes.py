...
class ClientRouteLayout(unittest.TestCase):
    ...
    def test_vercel_json_rewrites_onto_existing_functions(self):
        ...
        self.assertIn("/api/recipes", raw)
        self.assertIn("/api/dashboard?_r=recipes", raw)
        self.assertIn("/api/recipes/log", raw)
        self.assertIn("/api/dashboard?_r=recipes_log", raw)

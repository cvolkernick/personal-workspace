...
class CacheAndHobbyLock(unittest.TestCase):
    def test_static_cache_bumped(self):
        self.assertIn("?v=meal-micro-1", HTML)
        self.assertIn("?v=meal-slot-1", HTML)
        self.assertIn("?v=bottle-charge-7", HTML)
        self.assertIn("?v=paired-avgs-6", HTML)
        self.assertIn('const CACHE = "fitdash-shell-v96"', SW)
        self.assertIn("/styles.css?v=meal-micro-1", SW)
        ...
        self.assertIn("/app.js?v=quest-gt-local-1", SW)
        self.assertIn("/app.js?v=quest-gt-local-1", HTML)

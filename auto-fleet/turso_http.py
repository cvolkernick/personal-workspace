"""Stdlib HTTP client for Turso/libSQL. No local replica."""
...
def turso_url(env: Mapping[str, str] | None = None) -> str:
    src = env if env is not None else os.environ
    return (
        (src.get("AUTO_FLEET_TURSO_DATABASE_URL") or src.get("TURSO_DATABASE_URL") or "")
        .strip()
    )


def turso_token(env: Mapping[str, str] | None = None) -> str:
    src = env if env is not None else os.environ
    return (
        (src.get("AUTO_FLEET_TURSO_AUTH_TOKEN") or src.get("TURSO_AUTH_TOKEN") or "")
        .strip()
    )


def turso_enabled(env: Mapping[str, str] | None = None) -> bool:
    return bool(turso_url(env) and turso_token(env))
...
def connect(env: Mapping[str, str] | None = None) -> TursoConnection:
    if not turso_enabled(env):
        raise RuntimeError("TURSO_DATABASE_URL / TURSO_AUTH_TOKEN missing")
    return TursoConnection(turso_url(env), turso_token(env))

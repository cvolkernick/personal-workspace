...
| Layer | Role |
|--------|------|
| **Pi (systemd user units)** | Always-on backends on `0.0.0.0` for dashboards + business sites |
...
| Panamerica Auto (business) | **8795** | `panamerica-auto.service` |
...
# business site only:
bash deploy/install_remote.sh prism-agent@HOST --only panamerica
# or:
bash business/panamerica-auto/deploy/install_remote.sh prism-agent@HOST
# custom path:
...
curl -sS http://PI_LAN_IP:8795/ | head   # Panamerica Auto
# From this Mac terminal:

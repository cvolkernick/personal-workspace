"""Inbound Bland SMS/call webhook → CRM state machine."""
...
def extract_inbound(payload: dict[str, Any]) -> dict[str, str]:
    ...
    if outcome in {"completed", "interested"} and not str(text).strip():
        text = "interested"
    if outcome in {"no-answer", "no_answer", "voicemail"}:
        text = text or ""
    return {
...
def handle_payload(pipeline: Pipeline, payload: dict[str, Any]) -> dict[str, Any]:
    ...
    if inbound["outcome"] in {"no-answer", "no_answer", "voicemail"} and not inbound["text"]:
        if lead is None and inbound["phone"]:
            lead = pipeline.store.find_by_phone(inbound["phone"])
        if lead is None:
            return {"ignored": True, "reason": "unknown lead"}
        lead.notes.append(f"call outcome: {inbound['outcome']}")
        lead.events.append({"at": pipeline._iso(), "op": "call_no_answer", "outcome": inbound["outcome"]})
        pipeline.store.put(lead)
        return {"lead_id": lead.id, "state": lead.state, "outcome": inbound["outcome"]}
    ...
    class Handler(BaseHTTPRequestHandler):
        ...
        def do_GET(self) -> None:  # noqa: N802
            if self.path in {"/health", "/healthz"}:
                self._write(200, {"ok": True, "service": "panamerica-roadside-crm-webhook"})
                return

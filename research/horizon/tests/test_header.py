#!/usr/bin/env python3
"""Horizon header: FCC same-host deep-link; Orchestra and Seasonal Plan gone."""
...
class TestHorizonHeader(unittest.TestCase):
    ...
    def test_regime_ui_and_refresh_remain(self) -> None:
        html = (HORIZON / "index.html").read_text(encoding="utf-8")
        self.assertIn('id="regime-card"', html)
        self.assertIn('id="overview-regime"', html)
...
class TestHorizonHeaderLive(unittest.TestCase):
    ...
    def test_served_index_has_fcc_not_orchestra_or_seasonal(self) -> None:
        ...
        self.assertIn(b'id="regime-card"', body)

...
**Git (2026-09-11):** these machine files live on **`work/treasury`** (FCC live). They are **not** on `master` (FitDash). A clone of the GitHub default branch will look empty — that is the product branch map, not a missing-file bug. Do **not** recreate a second copy on `master`.

| File | Role | as_of |
|------|------|-------|
| [`fund_manager.json`](./fund_manager.json) | Machine policy: 60/40 sleeves, core allowlist, watchlist rules, `blocked_symbols` | 2026-09-11 |
| [`watchlist.json`](./watchlist.json) | Public consider-set (`ready` / `pass`). **Held is derived from FCC**, not this file | 2026-09-11 |
| [`private_watchlist.json`](./private_watchlist.json) | Pre-IPO monitor. Not deployable | 2026-09-07 |
| [`consider_share.json`](./consider_share.json) | Bias / new-money pins only (**TSLA 15 / SPCX 15**). Not NAV, not orders, not autopilot | 2026-09-08 (verified 2026-09-11) |
| [`positions.md`](./positions.md) | Human allowlist + current held names. Not a ledger | 2026-09-11 |
...
| **Energy** | Overarches BTC mining, AI infra, and hard-money macro | **BE pass / dropped** (owner exit 2026-09-03; blocked; do not reseat). **Held:** **CCJ** (first seat 2026-09-08). **Watch:** **BWXT** (Navy hardware). Not a basket. EXE not named |
...
- **Energy** is the shared constraint (mining, AI power, electrification); opportunistic, not a mandatory line-item. **BE** is **pass / dropped** (owner exit 2026-09-03; 50/50 TSLA+SPCX; `blocked_symbols`; 2026-09-08 $5 incident buy sold). **CCJ** first agentic seat 2026-09-08 — nuclear fuel + 49% Westinghouse, not a BE substitute. **BWXT** remains watchlist/ready (Navy/hardware). High ≠ first dollar. EXE / RRC / XIFR / Brookfield / CWEN **not** on the consider set.
...
**~60% stocks:** TSLA, SPCX (core); **held from watchlist:** GOOGL, NVDA, CCJ  

Energy: opportunistic (CCJ held; BWXT watch; BE blocked). Gold: sound-money under ~40%, **below BTC priority**, vehicle TBD, research-only until buy call.

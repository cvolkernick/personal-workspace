...
# SuperGrok / Grok Build session (same path FitDash uses)
GROK_AUTH_PATH = Path(
...
_HELP = (
    ...
    "bedroom 40% · dim master bedroom to 20% · master bedroom on · "
    "run sunset · status"
)
...
def parse_local(
    ...
    *,
    ...
    # brightness-only: "bedroom 40%", "dim bedroom to 20%", "set living room brightness 80"
    wants_bri_only = bool(
        re.search(
            r"\b(dim|bri(?:ghtness)?(?:\s+only)?|level|fade)\b",
            text,
        )
        or re.search(r"\d{1,3}\s*%", text)
    )
    ...
    elif not color and wants_bri_only:
        # leave color channel alone — Wiz PilotBuilder brightness only
        color = "keep"
    ...
    if target is None:
        if re.search(r"\b(all|everything|everywhere|house|home|lights)\b", text):
            target = "all"
        elif color and color not in ("keep", "off") and not re.search(
            r"\b(entry|living|bed|bath|shower|room|way|office|plant)\b", text
        ):
    ...
    if color and target:
        ...
        if color == "off":
            ...
            pct = max(0, min(100, int(round(bri * 100 / 255))))
            reply = f"Setting **{target}** brightness to **{pct}%** (color keep)."
        else:
    ...
    if target and not color:
        return {
            ...
            "engine": "local",
            "reply": (
                f"Got target **{target}** — add a color, on/off, or brightness "
                f"(e.g. {target} 40%). {_HELP}"
            ),
            "actions": [],
            "error": "missing_color",
        }
...
def _read_grok_auth_entry(path: Optional[Path] = None) -> Optional[dict[str, Any]]:
    """Load best SuperGrok session entry from ~/.grok/auth.json."""
    auth_path = path or GROK_AUTH_PATH
    ...
    for entry in data.values():
        ...
        if not token:
            continue
        exp = _parse_expires_at(entry.get("expires_at") if isinstance(entry.get("expires_at"), str) else None)
        if exp and exp <= now:
            if best is None:
                best = entry
            continue
...
def resolve_xai_credentials() -> dict[str, Any]:
    ...
    Priority:
      ...
      2. SuperGrok / Grok Build session in ~/.grok/auth.json (or GROK_AUTH_PATH)
    """
    ...
    if not entry:
        return {
            ...
            "expired": False,
            "error": (
                "No SuperGrok session. Sign in with `grok login` on the Pi host, "
                "or set XAI_API_KEY."
            ),
        }
    ...
    exp = _parse_expires_at(
        entry.get("expires_at") if isinstance(entry.get("expires_at"), str) else None
    )
    now = datetime.now(timezone.utc)
    expired = bool(exp and exp <= now)
    return {
        "token": token if token and not expired else None,
        "source": "supergrok_session",
        "email": entry.get("email"),
        "expired": expired,
        "error": (
            "SuperGrok session expired. Run `grok login` on the Pi, then retry."
            if expired
...
def _grok_available() -> bool:
    ...
    return bool(creds.get("token")) and not creds.get("expired")
...
def parse_with_grok(
    ...
    *,
    ...
    """Ask Grok for a structured action plan (XAI_API_KEY or SuperGrok session)."""
    creds = resolve_xai_credentials()
    api_key = (creds.get("token") or "").strip()
    if not api_key:
        return {
            "ok": False,
            "engine": "grok",
            "reply": creds.get("error") or "Grok unavailable (no credentials).",
            "actions": [],
            "error": "no_api_key",
    ...
    system = (
        ...
        '{"op":"run_routine","id":"sunset"|"sunrise"} | '
        '{"op":"status"} | {"op":"help"}. '
        "For brightness-only (keep color), use color \"keep\". "
